...
const MyAccount: NextPage = () => {
  ...
  const editHandler = async () => {
    if (edit) {
      const response = await changeRequest("post", "/api/users/username", {
        email: email,
        username: username,
      });
          ...
          <div className="relative flex flex-row">
            <div className="px-20 pt-10 pb-12 space-y-12 justify-center items-center flex flex-col">
              <div className="rounded-full w-80 h-80  border border-saffron flex justify-center items-center overflow-hidden">
                <img
                  className="w-80 h-80"
                  src="api/images/19e6ff94f579fef7891c55bf3d6faf11"
                />
              </div>
              <textarea
                placeholder="Write a fun description of yourself!"
                className="p-2 border border-saffron rounded-xl w-full"
                rows={5}
              />
            </div>
